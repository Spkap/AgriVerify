...
describe('AgriVerify contract', function () {
  ...
  describe('Transactions', function () {
    it('Should transfer tokens between accounts', async function () {
      const { agriVerify, owner, addr1, addr2 } = await loadFixture(
        deployAgriVerifyFixture,
      );
      await expect(
        agriVerify.transfer(addr1.address, ethers.parseUnits('50', 18)),
    ...
    it("Should fail if sender doesn't have enough tokens", async function () {
      const { agriVerify, owner, addr1 } = await loadFixture(
        deployAgriVerifyFixture,
      );
      ...
      expect(await agriVerify.balanceOf(owner.address)).to.equal(
        initialOwnerBalance,
      );
    });
  });
